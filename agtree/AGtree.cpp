...
AGtree::AGtree(DB *db_) : db(db_) {
    ...
    root->cache_dis.resize(db->num_data, 0);
}
...
void AGtree::crackInTwo(Node *node, float *query, float query_r, float *&dis, vector<float>& ans_dis) {
    ...
    while(true) {
        while (i <= node->end) {
            dis_i1 = dis[i];  // pivot1
            dis_i2 = calc_dis(db->dimension, db->data[i], pivot2);  // pivot2
            crack_calc_cnt ++;
        ...
        while (j >= node->start) {
            dis_j1 = dis[j];  // pivot1
            dis_j2 = calc_dis(db->dimension, db->data[j], pivot2);  // pivot2
            crack_calc_cnt ++;
...
void AGtree::search(Node *node, float* query, float query_r, float *&distance, vector<float>& ans_dis) {
    if (!node->is_leaf) {
        ...
        for (size_t i = 0; i < n; i ++ ) {
            dis[i] = calc_dis(db->dimension, node->pivots[i], query);
        }
        search_end
        search_calc_cnt += n;
        for (size_t i = 0; i < n; i ++ ) {
...
void AGtree::searchCache(Node *node, float* query, float query_r, float pq_dist, float *&distance, vector<float>& ans_dis) {
    if (!node->is_leaf) {
        ...
        if (node->end - node->start + 1 <= db->crack_threshold) {
            search_start
            for (int i = node->start; i <= node->end; i ++ ) {
                if (fabs(node->cache_dis[i - node->start] - pq_dist) > query_r) {
                    continue;
                }
                distance[i] = calc_dis(db->dimension, db->data[i], query);
                search_calc_cnt ++;
