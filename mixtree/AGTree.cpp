...
//                pivot_dis[i][j] = pivot_dis[j][i] = 0;
    ...
    crack_dis_cnt += sample_cnt * sample_cnt;   // todo
...
void MixTree::crackGInManyCache(Node *&node, float *query, float query_r, vector<float>& ans_dis) {
...
//        node->pivot_data_dist.shrink_to_fit();
    ...
    g_node->min_dist.resize(pivot_cnt, vector<float>(pivot_cnt, FLT_MAX));
    g_node->max_dist.resize(pivot_cnt, vector<float>(pivot_cnt, 0));
    ...
    for (int i = 0; i < data_cnt; i ++ ) {
        for (int j = 0; j < pivot_cnt; j ++ ) {
            tmp_dis[j] = dist(db->dimension, db->data[g_node->start + i], g_node->pivots[j]);   // 数据到第j个支枢点的距离
        ...
        for (int j = 0; j < pivot_cnt; j ++ ) {
            g_node->max_dist[j][closest_pivot] = max(g_node->max_dist[j][closest_pivot], tmp_dis[j]);    // 各个pivot到closest_pivot中最远的obj的距离
            g_node->min_dist[j][closest_pivot] = min(g_node->min_dist[j][closest_pivot], tmp_dis[j]);    // 各个pivot到closest_pivot中最近的obj的距离
        }
    }
